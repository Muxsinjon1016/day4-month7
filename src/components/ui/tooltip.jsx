...
export const Tooltip = ({ children, className, variant }) => {
  const variants = {
    ...
    header:
      "absolute py-2 px-4 w-[125px] text-center -bottom-[65px] headerTooltip headerTooltip -right-[25px] bg-gray-900 rounded-[12px]",
  };
  return (
    <div className={clsx(variants[variant], className)}>
      <div className="relative">
        <p className="text-xs text-white font-medium">{children}</p>
        <IoTriangle className="w-[30px] h-auto absolute -top-[32px] text-gray-900 right-[45px]" />
      </div>
    </div>
  );
};
